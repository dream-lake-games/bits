...
fn process_single_animation(def: &AnimDef, toml_content: &str) -> ProcessedAnim {
    ...
    let exports = batch_export_sprite_sheets(
        &absolute_file_str,
        &filtered_tags,
        &output_dir,
        &def.exclude,
...
fn generate_code(anims: &[ProcessedAnim]) -> String {
    ...
    for anim in anims {
        code.push_str(&format!(
            "    crate::bits_ui::anim::register_anim::<{}>(app);\n",
            anim.def.name
        ));
    }
    code.push_str("}\n");
    ...
    let table_entries: Vec<String> = anim
        ...
        .map(|(idx, v)| {
            ...
            let next_str = match &v.next {
                AnimNext::Loop => format!("crate::bits_ui::anim::AnimNextIndex::Index({})", idx),
                AnimNext::State(s) => {
                    let target_idx = variant_indices.get(s.as_str()).unwrap_or_else(|| {
                        panic!("Unknown next variant '{}' in {}", s, name);
                    });
                    format!("crate::bits_ui::anim::AnimNextIndex::Index({})", target_idx)
                }
                AnimNext::Remove => "crate::bits_ui::anim::AnimNextIndex::Remove".to_string(),
                AnimNext::Despawn => "crate::bits_ui::anim::AnimNextIndex::Despawn".to_string(),
            };
            format!(
                r#"    crate::bits_ui::anim::AnimVariant {{
        tag: "{}",
        fps: {},
        frame_count: {},
        frame_size: ({}, {}),
...
static {table_name}: &[crate::bits_ui::anim::AnimVariant] = &[
{table_entries}
];

impl crate::bits_ui::anim::Anim for {name} {{
    fn table() -> &'static [crate::bits_ui::anim::AnimVariant] {{
        {table_name}
    }}
...
fn batch_export_sprite_sheets(
    ...
    if is_cache_valid(file, &output_files) && Path::new(&cache_json_path).exists() {
        if let Ok(cache_content) = fs::read_to_string(&cache_json_path) {
...
"#,
    ...
    let cache_content = results
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let _ = fs::write(&cache_json_path, cache_content);
